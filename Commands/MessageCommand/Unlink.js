...
module.exports = {
    ...
    run: async (client, message) => {
        ...
        } catch (e) { console.warn('[unlink msg] DB lookup 실패:', e?.message ?? e) }
        ...
        const sent = await message.channel.send({ embeds: [confirm], components: [row] })
        ...
        collector.on('collect', async (button) => {
            ...
            try {
                try { removeSync(userFilePath(id)) } catch (e) { console.warn('[unlink msg] 파일 삭제 실패:', e?.message ?? e) }
                ...
                console.error('[unlink msg] failed:', err)
                await button.update({
